  ...
  /* ------------------------- publicação por plataforma ------------------------ */
  {
    ...
    description: describe(
      [
        ...
        '',
        'Publicações assíncronas (`202`) devolvem `jobId`: acompanhe em `GET .../publish/{jobId}` — a permissão e os erros de plataforma (escopo, conexão, vídeo inválido) aparecem lá, em `state: failed` + `code`.',
      ].join('\n'),
      `${crmAccess('social', 'CREATE')} Nas publicações assíncronas a permissão é conferida pelo worker, não na requisição.`,
    ),
