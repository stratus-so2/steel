...
export const GET = withAxiom(async (request: NextRequest, ctx: Params) => {
  ...
  const parsed = ListCrmTasksSchema.safeParse({
  ...
  const result = await CrmTaskService.list(auth.value.user.id, id, parsed.data)
...
export const POST = withAxiom(async (request: NextRequest, ctx: Params) => {
  ...
  const consent = await requireConsent(
    auth.value.user.id,
    'POST /api/workspaces/[id]/crm/tasks',
  )
  ...
  const result = await CrmTaskService.create(
    auth.value.user.id,
    id,
    parsed.data,
  )
