...
type Params = { params: Promise<{ id: string; definitionId: string }> }
...
export const PATCH = withAxiom(async (request: NextRequest, ctx: Params) => {
  ...
  const consent = await requireConsent(
    auth.value.user.id,
    'PATCH /api/workspaces/[id]/crm/custom-fields/[definitionId]',
  )
  ...
  const result = await CrmCustomFieldDefinitionService.update(
    auth.value.user.id,
    id,
    definitionId,
    parsed.data,
...
export const DELETE = withAxiom(async (_request: NextRequest, ctx: Params) => {
  ...
  const consent = await requireConsent(
    auth.value.user.id,
    'DELETE /api/workspaces/[id]/crm/custom-fields/[definitionId]',
  )
  if (!consent.ok) return handleError(consent.error)

  const { id, definitionId } = await ctx.params

  const result = await CrmCustomFieldDefinitionService.remove(
    auth.value.user.id,
    id,
    definitionId,
  )
