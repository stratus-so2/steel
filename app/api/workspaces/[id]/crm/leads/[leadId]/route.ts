...
export const GET = withAxiom(async (_request: NextRequest, ctx: Params) => {
  ...
  const result = await CrmLeadService.getById(auth.value.user.id, id, leadId)
...
export const PATCH = withAxiom(async (request: NextRequest, ctx: Params) => {
  ...
  const consent = await requireConsent(
    auth.value.user.id,
    'PATCH /api/workspaces/[id]/crm/leads/[leadId]',
  )
  ...
  const result = await CrmLeadService.update(
    auth.value.user.id,
    id,
    leadId,
    parsed.data,
...
export const DELETE = withAxiom(async (_request: NextRequest, ctx: Params) => {
  ...
  const consent = await requireConsent(
    auth.value.user.id,
    'DELETE /api/workspaces/[id]/crm/leads/[leadId]',
  )
  if (!consent.ok) return handleError(consent.error)

  const { id, leadId } = await ctx.params

  const result = await CrmLeadService.remove(auth.value.user.id, id, leadId)
  if (!result.ok) return handleError(result.error)
