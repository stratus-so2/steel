...
type Params = {
  params: Promise<{ id: string; leadId: string; proposalId: string }>
}
...
export const POST = withAxiom(async (request: NextRequest, ctx: Params) => {
  ...
  const consent = await requireConsent(
    auth.value.user.id,
    'POST /api/workspaces/[id]/crm/leads/[leadId]/proposal/[proposalId]/presentations',
  )
  ...
  const result = await CrmLeadService.registerProposalPresentation(
    auth.value.user.id,
    id,
    leadId,
    proposalId,
