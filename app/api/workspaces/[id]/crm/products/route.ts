...
export const GET = withAxiom(async (request: NextRequest, ctx: Params) => {
  ...
  const parsed = ListCrmProductsSchema.safeParse({
  ...
  const result = await CrmProductService.list(
    auth.value.user.id,
    id,
    parsed.data,
  )
...
export const POST = withAxiom(async (request: NextRequest, ctx: Params) => {
  ...
  const consent = await requireConsent(
    auth.value.user.id,
    'POST /api/workspaces/[id]/crm/products',
  )
  ...
  const parsed = CreateCrmProductSchema.safeParse(body)
  ...
  const result = await CrmProductService.create(
    auth.value.user.id,
    id,
    parsed.data,
  )
