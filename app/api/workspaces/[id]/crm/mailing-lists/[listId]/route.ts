...
export const PATCH = withAxiom(async (request: NextRequest, ctx: Params) => {
  ...
  const consent = await requireConsent(
    auth.value.user.id,
    'PATCH /api/workspaces/[id]/crm/mailing-lists/[listId]',
  )
  ...
  const result = await CrmMailingListService.update(
    auth.value.user.id,
    id,
    listId,
    parsed.data,
...
export const DELETE = withAxiom(async (_request: NextRequest, ctx: Params) => {
  ...
  const consent = await requireConsent(
    auth.value.user.id,
    'DELETE /api/workspaces/[id]/crm/mailing-lists/[listId]',
  )
  if (!consent.ok) return handleError(consent.error)

  const { id, listId } = await ctx.params

  const result = await CrmMailingListService.remove(
    auth.value.user.id,
    id,
    listId,
  )
