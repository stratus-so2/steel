...
export const GET = withAxiom(async (_request: NextRequest, ctx: Params) => {
  ...
  const result = await WhatsAppTemplateService.list(auth.value.user.id, id)
...
export const POST = withAxiom(async (request: NextRequest, ctx: Params) => {
  ...
  const consent = await requireConsent(
    auth.value.user.id,
    'POST /api/workspaces/[id]/whatsapp/templates',
  )
  ...
  const result = await WhatsAppTemplateService.create(
    auth.value.user.id,
    id,
    parsed.data,
  )
